...
def fp_from_gas(monthly_gas):
    ...
    1.1907
    '''
    emissions_lbs = monthly_gas * 105
    emissions_kg = pound_to_kg(emissions_lbs)
    return kg_to_tonnes(emissions_kg)
...
def fp_from_hydro(daily_hydro):
    ...
    0.0107
    '''
    emissions_by_hydro = (((daily_hydro / 1000) * 365) * 0.6)
    return kg_to_tonnes(emissions_by_hydro)
...
def fp_of_utilities(daily_hydro, monthly_gas):
    ...
    0.9635
    '''
    tonnes_annually = fp_from_gas(monthly_gas) + fp_from_hydro(daily_hydro)
    return tonnes_annually
...
def fp_of_studies(annual_uni_credits):
    ...
    0.672
    '''
    FTE_ratio = annual_uni_credits / 30
    student_emissions = FTE_ratio * 1.12
    return student_emissions
