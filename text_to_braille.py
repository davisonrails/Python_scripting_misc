...
def is_all_caps(s):
    ...
    if len(s) > 1:
        for i in range(len(s)):
            if is_letter(s[i]) and is_capitalized(s[i]):
                counter += 1
        if counter == len(s):
            return True
        return False
...
# This function takes each character and checks if it's a letter
# and if it's capitalized. If it's both, it adds to the counter.
# At the end, if the amount of capitalized letters is the same as
# the length of the string, then it's true, since all characters
# are proven to be capitalized letters.
...
def word_to_braille(word):
    ...
    if is_all_caps(word):
        ...
        for i in range(len(word)):
            if is_digit(word[i]):
                braille_word += NUMBER + '\n\n' + char_to_braille(word[i]) + '\n\n'
            elif is_letter(word[i]):
                if is_capitalized(word[i]):
                    braille_word += CAPITAL + '\n\n' + char_to_braille(word[i]) + '\n\n'
                else:
                    braille_word += char_to_braille(word[i]) + '\n\n'
            elif is_punctuation(word[i]):
                braille_word += char_to_braille(word[i]) + '\n\n'
            elif is_irregular(word[i]):
                braille_word += char_to_braille(word[i]) + '\n\n'

    return braille_word

# I broke the logic down to if it was all caps and if it wasn't. If it is all caps, it will
# add 2 braille capital constants  and then the rest of the world in Braille to the braille_word string.
# If it isn't all caps, it will iterate through each letter of the world, and depending on what type
# of character word[i] is, it will add the braille version to the braille_word string.
...
def new_filename(fname, addition):
    ...
    name_minus_extension = fname[0:len(fname)-4]
    return name_minus_extension + '_' + addition + extension
